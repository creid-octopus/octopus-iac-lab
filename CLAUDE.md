...
Rego rules Octopus evaluates against **every deployment and runbook run** before it executes. Verified working on self-hosted 2026.2.13392. (Policy *Evaluations* — the dry-run-against-past-executions tab — is an additive feature needing 2026.3.3064; the base policy engine does not.)

**They live as OCL in the Platform Hub repo**, at `.octopus/policies/<slug>.ocl`, exactly parallel to `.octopus/process-templates/<slug>.ocl`. Octopus commits them itself on save. Schema, confirmed by reading what it wrote:

```hcl
name             = "Manual intervention required"
description      = "..."
violation_action = "block"          # or "warn"
violation_reason = "..."
...
scope { rego = <<-EOT
    ...
    evaluate if { input.Environment.Name == "Production" }
EOT
}
```
...
- **The package name must equal the policy slug exactly**, and the rule names `evaluate` (scope) and `result` (conditions) are fixed. Since Rego package names can't contain hyphens, **edit the slug to underscores when creating the policy** — a default `manual-intervention-required` slug produces Rego that won't compile. The slug is immutable after creation.
- **`reason` and `action` in the Rego result override the OCL `violation_reason` / `violation_action`.** Pick one home for the wording. Setting both leaves a `violation_reason` that can never appear, which reads like a bug later. Leaving `action` out of the Rego is usually right: warn → block then becomes a metadata change rather than a Rego edit plus republish.
- **Lifecycle is commit → publish → activate, and all three are required.** Published-but-inactive policies are never evaluated. First publish must be a major version.
- **The input object describes the Octopus execution, not Kubernetes.** `Environment`, `Project`, `Space`, `ProjectGroup`, `Steps`, `SkippedSteps`, `Execution`, `RequiresApproval` always present; `Tenant` only for tenanted deployments, `Release` only for deployments, `Runbook` only for runbook runs — guard the last three before dereferencing. Full reference: [schema for policies](https://octopus.com/docs/platform-hub/policies/schema).
- **Enforcing "a required step must exist" needs three checks, not one.** A step that is skipped for this deployment still appears in `Steps` with its id added to `SkippedSteps`; a step *disabled in the process* appears in `Steps` and is **not** in `SkippedSteps`. So check `ActionType`, `Enabled`, and absence from `SkippedSteps`. Checking only `ActionType` means a disabled approval step satisfies the policy while never running.
- **The `.rego` files under `tofu/platform-hub/policies/` are NOT Platform Hub policies.** Seven of them are `package main` conftest rules that inspect *rendered Kubernetes manifests*, a domain Platform Hub never sees. They're conftest/Gatekeeper material. Only `worker_pool_gate.rego` is aimed at the process, and even that one is the wrong shape (it needs `evaluate`/`result` and a slug-matching package). Don't try to load that directory into Platform Hub.
- **Publish and activate have no documented API.** The equivalents for process templates weren't documented either and were found by tailing `docker compose logs octopus` while clicking the UI button; same trick applies if you want `make policy-publish` for policies-as-code.
...
- **Process Templates ≠ Project Templates** (two distinct features). Process Templates are reusable deployment-process *fragments* a project instantiates as steps, alongside its own steps. Project Templates template the *whole project*: the project inherits the template's process and variables and is customised only through parameter values.
  **Project Templates is in Public Preview and IS available here** — Enterprise on Cloud, and self-hosted on 2026.2+. Confirmed working on self-hosted 2026.2.13392. (An earlier note in this file said it was gated off with `Project templates feature toggle is not enabled`; that was true on 2026.1.x and is now stale.) Standard SLAs don't apply and Octopus doesn't recommend it for production workloads yet.
  Behaviour that matters when using it:
  - **A templated project cannot modify its inherited process.** No adding, removing, reordering, or disabling steps. That's the point — it pairs with Platform Hub Policies to make compliance structural rather than advisory — but it also means a team that trips a policy genuinely cannot fix it locally; the template owner has to ship a new version.
  - **Version acceptance is chosen at project creation**: *Accept minor changes* (patch + minor auto-apply) or *Accept patches* (patch only). Changeable later in project settings. Publishing a minor to a project set to "Accept patches" does nothing, which looks like a broken update.
  - **Major updates always require manual apply**, regardless of preference, and **block new releases** until reviewed. That's the intended forcing function for breaking changes.
  - Full reference: [templated projects](https://octopus.com/docs/platform-hub/templates/project-templates/using-project-templates).
